...
const CollegeForm = React.memo(({ form, uiRefresh, collegeMenus, alertRef, pageData, recordIndex, collegeAddedList }) => {
    ...
    const removeMajor = (idx) => {
        alertRef.current.showConfirm((res) => {
            ...
            subRefresh(Date.now());
            let params = [{ _modal: 'EductionList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { [`colleges.${recordIndex}.majors`]: { id: deleteid } } } }];
    ...
    const saveCollege = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'EductionList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'colleges': arr } } }] :
            [{ _modal: 'EductionList', _condition: 'update', _find: { _id: pageData.current._id, 'colleges.id': arr.id }, _data: { $set: { "colleges.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeCollege = () => {
        if (collegeAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'EductionList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'colleges': { id: formRef.current.id } } } }];
                apiPostCall('/api/common/common_mutiple_insert', { _list: params });
                collegeAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this college?');
