...
const InsuranceForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, propertyAddedList }) => {
    ...
    const completeHandler = (event) => {
        propertyAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveInsurance = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'propertypay': arr } } }] :
            [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id, 'propertypay.id': arr.id }, _data: { $set: { "propertypay.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...propertyAddedList.current];
                newlist[recordIndex] = arr;
                propertyAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Insurance added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Insurance and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            propertyAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeInsurance = () => {
        if (propertyAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                propertyAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Insurance?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'propertypay': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        <div className="flex w-full justify-start items-center relative">
                            ...
                            <label>Insurance Company</label>
                                <select
                                    className={`border w-full p-2 rounded ${!formRef.current.insuranceCompany ? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.insuranceCompany} onChange={e => { formRef.current.insuranceCompany = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value=""></option>
                                    {formList.insuranceCompany.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                </select>
                            </div>
                            <div className="w-1/3 mr-5">
                        ...
                        <div className="flex w-full justify-start items-center relative">
                            ...
                            <div className="w-1/3 mr-5">
                            <label>Wind and Hail $</label>
                            <input
                                    type="text"
                                    value={formRef.current.windhail}
                                    className={`w-full rounded border ${!formRef.current.windhail ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.windhail = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                        </div>
                        <div className="flex w-full justify-start items-center relative">
