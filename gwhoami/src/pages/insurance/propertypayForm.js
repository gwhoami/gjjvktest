...
const InsuranceForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, propertyAddedList }) => {
    ...
    const saveInsurance = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'property': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'property.id': arr.id }, _data: { $set: { "property.$": arr } }, _options: { upsert: false } }];
        (async () => {
