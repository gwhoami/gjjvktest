...
const MonthlypayForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, monthlypayAddedList }) => {
    ...
    const saveMonthlypay = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'monthlypay': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'monthlypay.id': arr.id }, _data: { $set: { "monthlypay.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeMonthlypay = () => {
        if (monthlypayAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'monthlypay': { id: formRef.current.id } } } }];
