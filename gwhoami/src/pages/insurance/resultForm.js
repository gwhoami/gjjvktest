...
const ResultForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, resultAddedList }) => {
    ...
    const completeHandler = (event) => {
        resultAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveResult = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'result': arr } } }] :
            [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id, 'result.id': arr.id }, _data: { $set: { "result.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...resultAddedList.current];
                newlist[recordIndex] = arr;
                resultAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Result added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the result and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            resultAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeResult = () => {
        if (resultAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                resultAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this result?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'result': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/2 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.resultName}
                                    className={`w-full rounded border ${!formRef.current.resultName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.resultName = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/2 mr-5">
                                <label>Collected On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.collectedDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.collectedDate ? new Date(formRef.current.collectedDate) : ''}
                                    onChange={date => { formRef.current.collectedDate = date; subRefresh(Date.now()); }}
                                />
                            </div>             
                        </div>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/2 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.resultValue}
                                    className={`w-full rounded border ${!formRef.current.resultValue ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.resultValue = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/2 mr-5">
                                <label>Order On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.orderDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.orderDate ? new Date(formRef.current.orderDate) : ''}
                                    onChange={date => { formRef.current.orderDate = date; subRefresh(Date.now()); }}
                                />
                           </div>
                        </div>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/2 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.resultUnit}
                                    className={`w-full rounded border ${!formRef.current.resultUnit ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.resultUnit = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/2 mr-5">
                                <label>Result On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.resultDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.resultDate ? new Date(formRef.current.resultDate) : ''}
                                    onChange={date => { formRef.current.resultDate = date; subRefresh(Date.now()); }}
                                />
                           </div>
                        </div>
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.resultComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    placeholder="http://totalhlth.followmyhealth.com/patientaccess#/Home"
                                    value={formRef.current.resultComments}
                                    onChange={e => { formRef.current.resultComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
