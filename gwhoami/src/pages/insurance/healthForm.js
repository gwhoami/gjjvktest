...
const HinsuranceForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, hinsuranceAddedList }) => {
    ...
    const saveHinsurance = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'health': arr } } }] :
            [{ _modal: 'InsuranceList', _condition: 'update', _find: { _id: pageData.current._id, 'health.id': arr.id }, _data: { $set: { "health.$": arr } }, _options: { upsert: false } }];
        (async () => {
