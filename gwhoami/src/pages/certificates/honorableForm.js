...
const HonorableForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, honorableAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
        honorableAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveHonorable = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'CertificateList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'honorable': arr } } }] :
            [{ _modal: 'CertificateList', _condition: 'update', _find: { _id: pageData.current._id, 'honorable.id': arr.id }, _data: { $set: { "honorable.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...honorableAddedList.current];
                newlist[recordIndex] = arr;
                honorableAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Honorable Details are added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Honorable details and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            honorableAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeHonorable = () => {
        if (honorableAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                honorableAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Honorable Details?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'CertificateList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'honorable': { id: formRef.current.id } } } }];
                    ...
                    <div className="flex w-full justify-start items-center relative">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.honorableComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.honorableComments}
                                    onChange={e => { formRef.current.honorableComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
