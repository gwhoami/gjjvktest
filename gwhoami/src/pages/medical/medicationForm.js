...
const MedicationForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, medicationAddedList }) => {
    ...
    const completeHandler = (event) => {
        medicationAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveMedication = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'medication': arr } } }] :
            [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id, 'medication.id': arr.id }, _data: { $set: { "medication.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...medicationAddedList.current];
                newlist[recordIndex] = arr;
                medicationAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Medication added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Medication and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            medicationAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeMedication = () => {
        if (medicationAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                medicationAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this medication?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'medication': { id: formRef.current.id } } } }];
                ...
                <div className="pt-5 pb-3">
                    <form>
                        ...
                        <div className="w-1/3 mr-5">
                            ...
                            <Datetime
                                    className={`w-full rounded ${!formRef.current.refill ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.refill ? new Date(formRef.current.refill) : ''}
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.medicationComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    placeholder="Comments if any http://totalhlth.followhealth.com/patientaccess#/Home"
                                    value={formRef.current.medicationComments}
                                    onChange={e => { formRef.current.medicationComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
