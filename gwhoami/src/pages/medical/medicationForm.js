...
const MedicationForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, medicationAddedList }) => {
    ...
    const saveMedication = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'medication': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'medication.id': arr.id }, _data: { $set: { "medication.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeMedication = () => {
        if (medicationAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'medication': { id: formRef.current.id } } } }];
