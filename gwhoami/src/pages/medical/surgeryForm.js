...
const SurgeryForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, surgeryAddedList }) => {
    ...
    const saveSurgery = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'surgery': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'surgery.id': arr.id }, _data: { $set: { "surgery.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeSurgery = () => {
        if (surgeryAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'surgery': { id: formRef.current.id } } } }];
