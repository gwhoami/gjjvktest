...
const SurgeryForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, surgeryAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
        surgeryAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveSurgery = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'surgery': arr } } }] :
            [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id, 'surgery.id': arr.id }, _data: { $set: { "surgery.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...surgeryAddedList.current];
                newlist[recordIndex] = arr;
                surgeryAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Surgery added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Surgery and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            surgeryAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeSurgery = () => {
        if (surgeryAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                surgeryAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this surgery?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'MedicalList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'surgery': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        <div className="flex w-full justify-start items-center relative">
                            ...
                            <label>Type of Surgery</label>
                                <select
                                    className={`border w-full p-2 rounded ${!formRef.current.surgeryType ? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.surgeryType} onChange={e => { formRef.current.surgeryType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value=""></option>
                                    {formList.surgeryType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                </select>
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Surgery On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.surgeryDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.SurgeryDate ? new Date(formRef.current.SurgeryDate) : ''}
                                    onChange={date => { formRef.current.SurgeryDate= date; subRefresh(Date.now()); }}
                                />
                            </div>
                        </div>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.surgeryUnit}
                                    className={`w-full rounded border ${!formRef.current.surgeryUnit ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.surgeryUnit = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Discharge On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.dischargeDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.dischargeDate ? new Date(formRef.current.dischargeDate) : ''}
                                    onChange={date => { formRef.current.dischargeDate= date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Followup On</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.followupDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.followupDate ? new Date(formRef.current.followupDate) : ''}
                                    onChange={date => { formRef.current.followupDate= date; subRefresh(Date.now()); }}
                                />
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <InputRadio 
                                ...
                                formKey="insuranceHelp" 
                                formRef={regRef} 
                                ui={ui} 
                                label="Insurance Helped"
                                name="insuranceHelp" 
                                values={['Yes', 'No']} 
                                required="Insurance Helped is required" 
                            />    
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Insurance covered percent</label>
                            <input
                                    type="text"
                                    value={formRef.current.insureCovered}
                                    className={`w-full rounded border ${!formRef.current.insureCovered ? 'border-red-500 err-input' : 'border-gray-400'}`}
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.surgeryComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.surgeryComments}
                                    onChange={e => { formRef.current.surgeryComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
