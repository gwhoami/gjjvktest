...
const ImmuneForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, immuneAddedList }) => {
    ...
    const saveImmune = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'immune': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'immune.id': arr.id }, _data: { $set: { "immune.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeImmune = () => {
        if (immuneAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'immune': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Dose Name</label>
                                <select
                                    className={`border w-full p-2 rounded ${!formRef.current.doserName ? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.doseName} onChange={e => { formRef.current.doseName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value=""></option>
