...
const MedicalTabs = React.memo(() => {
    ...
    useEffect(() => {
        (async () => {
            let search = [{ _modal: 'ProfileList', _find: { userid: MyLocalStorage.getUserId() }, _mode: 'single', _select: 'general immune allergi healthinfo surgery medication' }];
            ...
            if (res.isError) {
                ...
                if (res && res.length === 0) {
                    const newrecord = await apiPostCall('/api/common/common_mutiple_insert', { _list: [{ _modal: 'ProfileList', _condition: 'new', _data: { userid: MyLocalStorage.getUserId(), general: [], immune: [], allergi: [], healthinfo: [], surgery: [], medication: [] } }] });
    ...
    if (!pageData.current.init) return <div className="flex w-full h-full justify-center items-center"><Spinner size="60" /></div>
        ...
        <div className="flex px-6 w-full container justify-center mx-auto pb-5">
            <div className="sm:w-full md:w-full xl:w-3/5 mt-20">
                <Tabs
                    selectedTabKey={tabid === 'general' ? 0 : tabid === 'immune' ? 1 : tabid === 'allergi' ? 2 :  tabid === 'healthinfo' ? 3 : tabid === 'surgery' ? 4 :  tabid === 'medication' ? 5 : 0}
                    transformWidth={600}
