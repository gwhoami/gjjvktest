...
const RegularForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, regularAddedList }) => {
    ...
    const saveRegular = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'regular': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'regular.id': arr.id }, _data: { $set: { "regular.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeRegular = () => {
        if (regularAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'regular': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        ...
                        <div className="flex w-full justify-start items-center relative">
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.bodyMI}
                                    className={`w-full rounded border ${!formRef.current.bodyMI ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.bodyMI = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3">
