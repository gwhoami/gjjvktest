...
const AllergiesForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, allergiAddedList }) => {
    ...
    const saveAllergi = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'allergi': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'allergi.id': arr.id }, _data: { $set: { "allergi.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeAllergi = () => {
        if (allergiAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'allergi': { id: formRef.current.id } } } }];
