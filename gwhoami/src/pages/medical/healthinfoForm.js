...
const HealthinfoForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, healthinfoAddedList }) => {
    ...
    const saveHealthinfo = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'helthinfo': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'healthinfo.id': arr.id }, _data: { $set: { "healthinfo.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeHealthinfo = () => {
        if (healthinfoAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'Healthinfo': { id: formRef.current.id } } } }];
