...
const IncomeForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, incomeAddedList }) => {
    ...
    const completeHandler = (event) => {
        incomeAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveIncome = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'FinancialList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'income': arr } } }] :
            [{ _modal: 'FinancialList', _condition: 'update', _find: { _id: pageData.current._id, 'income.id': arr.id }, _data: { $set: { "income.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...incomeAddedList.current];
                newlist[recordIndex] = arr;
                incomeAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Income Details are added succesfully!', timeout: 1200 });
            }
        })();
    }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            incomeAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeIncome = () => {
        if (incomeAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                incomeAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Certificate Details?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'FinancialList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'income': { id: formRef.current.id } } } }];
