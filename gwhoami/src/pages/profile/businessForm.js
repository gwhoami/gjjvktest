...
const BusinessForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, businessAddedList }) => {
    ...
    const completeHandler = (event) => {
        businessAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveBusiness = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'business': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'business.id': arr.id }, _data: { $set: { "business.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...businessAddedList.current];
                newlist[recordIndex] = arr;
                businessAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Business added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the business and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            businessAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeBusiness = () => {
        if (businessAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                businessAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this business?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'business': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.businessName}
                                    className={`w-full rounded border ${!formRef.current.businessName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.businessName = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.businessTitle}
                                    className={`w-full rounded border ${!formRef.current.businessTitle ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.businessTitle = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3">
                                <label>Business Type</label>
                                <select className={`border w-full p-2 rounded ${!formRef.current.businessType ? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.businessType} onChange={e => { formRef.current.businessType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value=""></option>
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-1/3">
                                <label>Salary</label>
                                <CurrencyInput
                                    value={formRef.current.salary}
                                    className={`w-full rounded border ${!formRef.current.salary ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    intlConfig={{ locale: formList.localeByCountry[formRef.current.country], currency: formList.currencyByCountry[formRef.current.country] }}
                                    onValueChange={(value) => { formRef.current.salary = value; subRefresh(Date.now()); }}
                                />
                            </div>
                        </div>
                        <div className="flex w-full justify-start items-center mt-3">
                            <label>Is this your recent business?</label>
                            <div className="flex ml-5">
                                <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isRecentBusiness}
                                        onChange={e => { formRef.current.isRecentBusiness = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.comments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.comments}
                                    onChange={e => { formRef.current.comments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
