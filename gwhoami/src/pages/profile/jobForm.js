...
const JobForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, jobAddedList }) => {
    ...
    const completeHandler = (event) => {
        jobAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveJob = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'job': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'job.id': arr.id }, _data: { $set: { "job.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...jobAddedList.current];
                newlist[recordIndex] = arr;
                jobAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Job added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the job and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            jobAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeJob = () => {
        if (jobAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                jobAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this job?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'job': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        <div className="flex w-full justify-start items-center relative">
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.companyName}
                                    className={`w-full rounded border ${!formRef.current.companyName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.companyName = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                    type="text"
                                    value={formRef.current.jobTitle}
                                    className={`w-full rounded border ${!formRef.current.jobTitle ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    onChange={e => { formRef.current.jobTitle = e.currentTarget.value; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3">
                                <label>Job Type</label>
                                <select className={`border w-full p-2 rounded ${!formRef.current.jobType ? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.jobType} onChange={e => { formRef.current.jobType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value=""></option>
                                    {formList.jobType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                </select>
                            </div>
                        </div>
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <label>Is this your recent job?</label>
                            <div className="flex ml-5">
                                <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isRecentJob}
                                        onChange={e => { formRef.current.isRecentJob = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
