...
const GeneralForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, generalAddedList }) => {
    ...
    const saveGeneral = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'general': arr } } }] :
            [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id, 'general.id': arr.id }, _data: { $set: { "general.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Regular added succesfully!', timeout: 1200 });
            }
        })();
    }
    ...
    const removeGeneral = () => {
        if (generalAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'general': { id: formRef.current.id } } } }];
                    ...
                    <div className="flex w-full justify-start items-center relative">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.generalComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.generalComments}
                                    onChange={e => { formRef.current.generalComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
