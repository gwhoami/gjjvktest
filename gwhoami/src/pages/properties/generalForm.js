...
const GeneralForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, generalAddedList }) => {
    ...
    const saveGeneral = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'general': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'general.id': arr.id }, _data: { $set: { "general.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeGeneral = () => {
        if (generalAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'general': { id: formRef.current.id } } } }];
