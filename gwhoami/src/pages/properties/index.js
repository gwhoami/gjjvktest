...
const PropertiesTabs = React.memo(() => {
    ...
    useEffect(() => {
        (async () => {
            let search = [{ _modal: 'ProfileList', _find: { userid: MyLocalStorage.getUserId() }, _mode: 'single', _select: 'general house houseitems vehicles' }];
            ...
            if (res.isError) {
                ...
                if (res && res.length === 0) {
                    const newrecord = await apiPostCall('/api/common/common_mutiple_insert', { _list: [{ _modal: 'ProfileList', _condition: 'new', _data: { userid: MyLocalStorage.getUserId(), general: [], house: [], vehicles: [], houseitems: [] } }] });
    ...
    if (!pageData.current.init) return <div className="flex w-full h-full justify-center items-center"><Spinner size="60" /></div>
        ...
        <div className="flex px-6 w-full container justify-center mx-auto pb-5">
            <div className="sm:w-full md:w-full xl:w-3/5 mt-20">
                <Tabs
                    selectedTabKey={tabid === 'general' ? 0 : tabid === 'house' ? 1 : tabid === 'vehicles' ? 2 : tabid === 'houseitems' ? 3 : 0} 
                    transformWidth={600}
