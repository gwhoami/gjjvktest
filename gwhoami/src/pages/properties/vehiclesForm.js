...
const VehiclesForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, vehiclesAddedList }) => {
    const [ui] = useState(-1);
    ...
    const saveVehicles = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'vehicles': arr } } }] :
            [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id, 'vehicles.id': arr.id }, _data: { $set: { "vehicles.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Vehicles added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Vehicles and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeVehicles = () => {
        if (vehiclesAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'vehicles': { id: formRef.current.id } } } }];
