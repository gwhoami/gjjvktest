...
const HouseForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, houseAddedList }) => {
    ...
    const saveHouse = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'house': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'house.id': arr.id }, _data: { $set: { "house.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeHouse = () => {
        if (houseAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'house': { id: formRef.current.id } } } }];
