...
const HouseitemsForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, houseitemsAddedList }) => {
    ...
    const saveHouseitems = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'houseitems': arr } } }] :
            [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id, 'houseitems.id': arr.id }, _data: { $set: { "houseitems.$": arr } }, _options: { upsert: false } }];
        (async () => {
    ...
    const removeHouseitems = () => {
        if (houseitemsAddedList.current[recordIndex].saved === false) {
            ...
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'ProfileList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'houseitems': { id: formRef.current.id } } } }];
