...
const HouseitemsForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, houseitemsAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
        houseitemsAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveHouseitems = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'houseitems': arr } } }] :
            [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id, 'houseitems.id': arr.id }, _data: { $set: { "houseitems.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...houseitemsAddedList.current];
                newlist[recordIndex] = arr;
                houseitemsAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Houseitems added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the houseitems and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            houseitemsAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeHouseitems = () => {
        if (houseitemsAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                houseitemsAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this houseitems?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'PropertyList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'houseitems': { id: formRef.current.id } } } }];
                    ...
                    <form>
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.houseitemsComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.houseitemsComments}
                                    onChange={e => { formRef.current.houseitemsComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
